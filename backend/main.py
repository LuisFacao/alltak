...
class PayslipModel(Base):
    ...
    # Arquivo armazenado como bytes.
    # O helper abaixo também trata registros antigos
    # que eventualmente estejam salvos como Base64/string.
...
def _payslip_bytes(file_data) -> bytes:
    """
    Converte o conteúdo armazenado do holerite para bytes.

    Suporta:
    - bytes / bytearray / memoryview: arquivo binário direto
    - string Base64
    - data URL Base64
    """
    ...
    if isinstance(file_data, (bytearray, memoryview)):
        return bytes(file_data)

    if isinstance(file_data, str):
        value = file_data.strip()
        ...
        try:
            ...
            # Compatibilidade com registros antigos que possam
            # ter sido armazenados como texto.
            return value.encode("utf-8")

    return bytes(file_data)


def _payslip_base64(file_data) -> Optional[str]:
    data = _payslip_bytes(file_data)
...
def serialize_payslip(p):
    """
    Retorna os dados do holerite em JSON.
    O arquivo é convertido para Base64 apenas na resposta JSON.
    O download real usa os bytes diretamente.
    """
    return {
        "id": p.id,
        ...
        "file_data": _payslip_base64(p.file_data),
        "created_at": (
            p.created_at.isoformat()
            if p.created_at
            else None
...
from rotas.feedback import (
    feedback_router,
    direct_feedback_router,
)
...
def read_root():
    return {
        "status": "API online e rodando! "
                  "(SQLite local, sem Supabase)"
    }
