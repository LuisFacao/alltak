...
# ---------------------------------------------------------------------------
# CONFIG — no external service required. SQLite file is created automatically.
# ---------------------------------------------------------------------------
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///./alltak.db")
...
# ---------------------------------------------------------------------------
# SCHEMAS — unchanged shapes, so the frontend needs no edits here
# ---------------------------------------------------------------------------
...
async def create_user(data: UserData, db: Session = Depends(get_db)):
    ...
    user = UserModel(email=data.email, password_hash=pwd_context.hash(data.password), role=data.role, initial=initial)
    db.add(user)
    db.commit()
    db.refresh(user)
