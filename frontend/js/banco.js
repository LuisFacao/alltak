...
export const Database = {
  async _apiRequest(
    ...
    try {
      ...
      console.warn(
        `[API Helper] Falha ao conectar em ${path}:`,
        error.message
      );
  ...
  async login(email, password) {
    return this._apiRequest(
      "/auth/login",
      this._apiJsonOptions("POST", { email, password }),
  ...
  async getDirectfeedback(recipient) {
    const path = recipient
      ? `/direct-feedback?recipient=${encodeURIComponent(recipient)}`
      : "/direct-feedback";

    return this._apiRequest(
      path,
      {},
      "Erro ao buscar feedback direcionados"
    ).catch(() => []);
  },

  async createDirectfeedback(
    recipient,
    message,
    attachments
  ) {
    return this._apiRequest(
      ...
      "Erro ao enviar feedback direcionado"
    );
  },

  async uploadDirectFeedback(
    recipient,
    message,
    fileList
  ) {
  ...
  async deleteDirectfeedbackApi(id) {
    return this._apiRequest(
      `/direct-feedback/${id}`,
      { method: "DELETE" },
      "Erro ao excluir feedback direto",
      { parseJson: false }
    );
  },
  ...
  async getHolerite(recipient) {
    const path = recipient
      ? `/payslips?recipient=${encodeURIComponent(recipient)}`
      : "/payslips";

    return this._apiRequest(
      path,
      {},
      "Erro ao buscar holerites"
    ).catch(() => []);
  },

  // Abre/visualiza o PDF
  async getHoleriteFile(id) {
    return this._apiRequest(
      `/payslips/${id}/file`,
      {},
      "Erro ao buscar arquivo do holerite",
      { responseType: "blob" }
    );
  },
  ...
  async updateHolerite(
    ...
    return this._apiRequest(
      `/payslips/${id}`,
      this._apiJsonOptions("PUT", {
        recipient,
        ref
      }),
      "Erro ao atualizar holerite"
    );
  },
